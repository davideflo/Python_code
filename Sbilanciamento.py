...
def SbilPriceGetter():
    ...
    for y in years:    
        for m in mon:
            print(m)
            if y == 2017 and m in ['04', '05', '06', '07', '08', '09', '10', '11', '12']:
                break
            else:
                ...
                sbil_tot = sbil_tot.append(sbil[['CODICE RUC', 'DATA RIFERIMENTO CORRISPETTIVO', 
                'SEGNO SBILANCIAMENTO AGGREGATO ZONALE',
                '(Pmgp_ven) PREZZO VALORIZZAZIONE OFFERTE VENDITA MGP [Euro/MWh]', 
                '(Pmgp_acq) PREZZO VALORIZZAZIONE OFFERTE ACQUISTO MGP [Euro/MWh]',
                '(PmedioMSD_acq) PREZZO MEDIO OFFERTE ACQUISTO MSD [Euro/MWh]', 
                '(PmedioMSD_ven)PREZZO MEDIO OFFERTE VENDITA MSD [Euro/MWh]']], ignore_index = True)                
                
    sbil_tot.columns = [['CODICE RUC', 'DATA RIFERIMENTO CORRISPETTIVO','SEGNO SBILANCIAMENTO AGGREGATO ZONALE', 
                '(Pmgp_ven)', 
                '(Pmgp_acq)',
                '(PmedioMSD_acq)', 
                '(PmedioMSD_ven)']]
    
    sbil_tot.to_excel("valorizzazione_sbilanciamento.xlsx")    
    return sbil_tot
###############################################################################
...
sbil_tot.to_excel('aggregato_sbilanciamento2.xlsx')        
