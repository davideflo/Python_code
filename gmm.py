...
def gmm_init(X, K, verbose = False,
    ...
    -----------
    cluster_init : string
        How to initalize centroids: 'sample', 'box', 'custom' or 'kmeans'
    ...
    elif cluster_init == 'custom':
        center_list = []
        if cntr_lst == []:
            raise ValueError("centroid list must be provided.")
        else:
            if verbose: print "Using user provided centroids."
            center_list = cntr_lst
